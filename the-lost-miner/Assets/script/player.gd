...
@onready var deall_damage_zone: Area2D = $DeallDamageZone
...
func _on_damage_zone_body_entered(body):
	if is_attacking and body.has_method("take_damage"):
		body.take_damage(20)  # deal 20 damage (you can tweak)
		
func deal_damage():
	for body in deall_damage_zone.get_overlapping_bodies():
		if body.has_method("take_damage"):
			body.take_damage(20)

func _ready() -> void:
	weapon_equip = false
	deall_damage_zone.connect("body_entered", Callable(self, "_on_damage_zone_body_entered"))
